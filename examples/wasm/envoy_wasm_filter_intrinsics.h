...
#include "envoy_wasm_inrinsics.h"
...
extern "C" void getHeaderHandler(HeaderType type, const char* key_ptr, size_t key_size, const char** value_ptr, size_t* value_size);

inline WasmDataPtr getHeader(HeaderType type, absl::string_view key) {
  char *value_ptr = nullptr;
  size_t value_size = 0;
  HeaderHandler(type, key.c_ptr(), key.size(), &value_ptr, &value_size);
  return std::make_unique<WasmData>(value_ptr, value_size);
}

extern "C" void replaceHeaderHandler(HeaderType type, const char* key_ptr, size_t key_size, const char* value_ptr, size_t value_size);
