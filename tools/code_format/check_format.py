...
def checkTools():
    ...
    if clang_format_abs_path:
        ...
        error_messages.append(
            ...
            "    export CLANG_FORMAT=/usr/local/opt/llvm@10/bin/clang-format".
    ...
    if not re.search("^\s*namespace\s+%s\s*{" % namespace_check, text, re.MULTILINE) and \
       not nolint in text:
        return [
            ...
            (namespace_check, nolint, file_path)
        ]
    return []
...
def errorIfNoSubstringFound(pattern, file_path, error_message):
    return [] if pattern in readFile(file_path) else [
        file_path + ": " + error_message
    ]


def isApiFile(file_path):
    return file_path.startswith(args.api_prefix) or file_path.startswith(
        args.api_shadow_prefix)
...
def isExternalBuildFile(file_path):
    return isBuildFile(file_path) and (
...
def checkCurrentReleaseNotes(file_path, error_messages):
    ...
    for line_number, line in enumerate(readLines(file_path)):
        ...
        def reportError(message):
            error_messages.append("%s:%d: %s" % (file_path, line_number + 1,
                                                 message))
        ...
        if flag_match:
            if not flag_match.groups()[0].startswith('`'):
                reportError("Flag `%s` should be enclosed in back ticks" %
                            flag_match.groups()[1])
...
def checkSourceLine(line, file_path, reportError):
    ...
    if isInSubdir(file_path, 'source',
    ...
    if not allowlistedForSerializeAsString(
    ...
    if not allowlistedForJsonStringToMessage(
    ...
    if not allowlistedForGrpcInit(file_path):
        ...
        if grpc_init_or_shutdown == -1 or (
                grpc_shutdown != -1 and grpc_shutdown < grpc_init_or_shutdown):
...
def checkBuildLine(line, file_path, reportError):
    if not isBazelToolsCheckExcludedFile(
            file_path) and "@bazel_tools" in line and not (
    ...
    if (envoy_build_rule_check and not isStarlarkFile(file_path)
            and not isWorkspaceFile(file_path)
            and not isExternalBuildFile(file_path) and "@envoy//" in line):
...
def fixBuildLine(file_path, line, line_number):
    if (envoy_build_rule_check and not isStarlarkFile(file_path)
            and not isWorkspaceFile(file_path)
            and not isExternalBuildFile(file_path)):
...
def fixBuildPath(file_path):
    ...
    if not isBuildFixerExcludedFile(file_path) and not isApiFile(
            file_path) and not isStarlarkFile(
...
def checkBuildPath(file_path):
    ...
    if isBuildFile(file_path) and (
            file_path.startswith(args.api_prefix + "envoy")
            or file_path.startswith(args.api_shadow_prefix + "envoy")):
        found = False
    ...
    error_messages += executeCommand(command, "buildifier check failed",
                                     file_path)
    ...
    if not file_path.endswith(DOCS_SUFFIX):
        if not file_path.endswith(PROTO_SUFFIX):
            ...
            command = ("%s --include_dir_order %s --path %s | diff %s -" %
                       (HEADER_ORDER_PATH, include_dir_order, file_path,
                        file_path))
            error_messages += executeCommand(
                command, "header_order.py check failed", file_path)
        command = ("%s %s | diff %s -" % (CLANG_FORMAT_PATH, file_path,
                                          file_path))
        error_messages += executeCommand(command, "clang-format check failed",
                                         file_path)
...
def checkFormat(file_path):
    ...
    if isBuildFile(file_path) or isStarlarkFile(file_path) or isWorkspaceFile(
            file_path):
        if try_to_fix:
...
def checkApiShadowStarlarkFiles(api_shadow_root, file_path, error_messages):
    command = "diff -u "
    command += file_path + " "
    api_shadow_starlark_path = api_shadow_root + re.sub(
        r"\./api/", '', file_path)
    command += api_shadow_starlark_path

    error_message = executeCommand(
...
def checkFormatVisitor(arg, dir_name, names):
  ...
  """
    ...
    if dir_name.startswith(full_prefix) and '/' in dir_name[len(full_prefix):]:
        checkOwners(dir_name[len(source_prefix):], owned_directories,
                    error_messages)

    for file_name in names:
        if dir_name.startswith("./api") and isStarlarkFile(file_name):
            result = pool.apply_async(
                checkApiShadowStarlarkFiles,
                args=(api_shadow_root, dir_name + "/" + file_name,
                      error_messages))
            result_list.append(result)
        result = pool.apply_async(
            checkFormatReturnTraceOnError, args=(dir_name + "/" + file_name, ))
        result_list.append(result)
...
if __name__ == "__main__":
    ...
    def ownedDirectories(error_messages):
        ...
        maintainers = [
            ...
            '@snowp', '@asraa', '@yavlasov', '@junr03', '@dio', '@jmarantz'
        ]
    ...
    if os.path.isfile(target_path):
        ...
        def PooledCheckFormat(path_predicate):
            ...
            for root, _, files in os.walk(target_path):
                checkFormatVisitor((pool, results, owned_directories,
                                    api_shadow_root, error_messages), root,
