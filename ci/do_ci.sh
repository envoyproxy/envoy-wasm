...
if [[ "$1" == "fix_format" || "$1" == "check_format" || "$1" == "check_repositories" || \
        ...
        "$1" == "check_spelling_pedantic" || "$1" == "fix_spelling_pedantic" || "$1" == "bazel.asan" ]]; then
...
if [[ "$CI_TARGET" == "bazel.release" ]]; then
  ...
  setup_clang_toolchain
  BAZEL_BUILD_OPTIONS="${BAZEL_BUILD_OPTIONS} -c opt --copt -g --config=clang-asan --build_tests_only"
