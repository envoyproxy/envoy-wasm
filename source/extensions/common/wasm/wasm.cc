...
public:
  ...
  uint32_t nextCas() { auto result = cas; cas++; if (!cas) cas++; return result; }

private:
  absl::Mutex mutex;
  uint32_t cas = 1;
  absl::node_hash_map<std::string, absl::flat_hash_map<std::string, std::pair<std::string, uint32_t>>> data;
};
...
void getRequestStreamInfoProtocolHandler(void *raw_context, uint32_t value_ptr_ptr, uint32_t value_size_ptr) {
  ...
  context->wasmVm()->copyToPointerSize(context->getRequestStreamInfoProtocol(), value_ptr_ptr, value_size_ptr);
}

void getResponseStreamInfoProtocolHandler(void *raw_context, uint32_t value_ptr_ptr, uint32_t value_size_ptr) {
  auto context = WASM_CONTEXT(raw_context);
  context->wasmVm()->copyToPointerSize(context->getResponseStreamInfoProtocol(), value_ptr_ptr, value_size_ptr);
}
...
void getRequestMetadataHandler(void *raw_context, uint32_t key_ptr, uint32_t key_size, uint32_t value_ptr_ptr, uint32_t value_size_ptr) {
  auto context = WASM_CONTEXT(raw_context);
  context->wasmVm()->copyToPointerSize(context->getRequestMetadata(context->wasmVm()->getMemory(key_ptr, key_size)), value_ptr_ptr, value_size_ptr);
}
...
void setRequestMetadataHandler(void *raw_context, uint32_t key_ptr, uint32_t key_size, uint32_t value_ptr, uint32_t value_size) {
  auto context = WASM_CONTEXT(raw_context);
  context->setRequestMetadata(context->wasmVm()->getMemory(key_ptr, key_size), context->wasmVm()->getMemory(value_ptr, value_size));
}
...
void getResponseMetadataHandler(void *raw_context, uint32_t key_ptr, uint32_t key_size, uint32_t value_ptr_ptr, uint32_t value_size_ptr) {
  auto context = WASM_CONTEXT(raw_context);
  context->wasmVm()->copyToPointerSize(context->getResponseMetadata(context->wasmVm()->getMemory(key_ptr, key_size)), value_ptr_ptr, value_size_ptr);
}
...
void setResponseMetadataHandler(void *raw_context, uint32_t key_ptr, uint32_t key_size, uint32_t value_ptr, uint32_t value_size) {
  auto context = WASM_CONTEXT(raw_context);
  context->setResponseMetadata(context->wasmVm()->getMemory(key_ptr, key_size), context->wasmVm()->getMemory(value_ptr, value_size));
}
...
void addRequestHeaderHandler(void *raw_context, uint32_t key_ptr, uint32_t key_size, uint32_t value_ptr, uint32_t value_size) {
  auto context = WASM_CONTEXT(raw_context);
  context->addRequestHeader(context->wasmVm()->getMemory(key_ptr, key_size), context->wasmVm()->getMemory(value_ptr, value_size));
}
...
void addRequestTrailerHandler(void *raw_context, uint32_t key_ptr, uint32_t key_size, uint32_t value_ptr, uint32_t value_size) {
  auto context = WASM_CONTEXT(raw_context);
  context->addRequestTrailer(context->wasmVm()->getMemory(key_ptr, key_size), context->wasmVm()->getMemory(value_ptr, value_size));
}
...
void addResponseHeaderHandler(void *raw_context, uint32_t key_ptr, uint32_t key_size, uint32_t value_ptr, uint32_t value_size) {
  auto context = WASM_CONTEXT(raw_context);
  context->addResponseHeader(context->wasmVm()->getMemory(key_ptr, key_size), context->wasmVm()->getMemory(value_ptr, value_size));
}
...
void addResponseTrailerHandler(void *raw_context, uint32_t key_ptr, uint32_t key_size, uint32_t value_ptr, uint32_t value_size) {
  auto context = WASM_CONTEXT(raw_context);
  context->addResponseTrailer(context->wasmVm()->getMemory(key_ptr, key_size), context->wasmVm()->getMemory(value_ptr, value_size));
}
...
void logHandler(void* raw_context, uint32_t level, uint32_t address, uint32_t size) {
  auto context = WASM_CONTEXT(raw_context);
  context->scriptLog(static_cast<spdlog::level::level_enum>(level), context->wasmVm()->getMemory(address, size));
}
...
absl::string_view Context::getRequestHeader(absl::string_view key) {
  if (access_log_request_headers_) return getHeader(access_log_request_headers_, key);
  return getHeader(request_headers_, key);
}

Pairs Context::getRequestHeaderPairs() {
  if (access_log_request_headers_) return getHeaderPairs(access_log_request_headers_);
...
absl::string_view Context::getRequestTrailer(absl::string_view key) {
  if (access_log_request_trailers_) return getHeader(access_log_request_trailers_, key);
  return getHeader(request_trailers_, key);
}

Pairs Context::getRequestTrailerPairs() {
  if (access_log_request_trailers_) return getHeaderPairs(access_log_request_trailers_);
...
absl::string_view Context::getResponseHeader(absl::string_view key) {
  if (access_log_response_headers_) return getHeader(access_log_response_headers_, key);
  return getHeader(response_headers_, key);
}

Pairs Context::getResponseHeaderPairs() {
  if (access_log_response_headers_) return getHeaderPairs(access_log_response_headers_);
...
absl::string_view Context::getResponseTrailer(absl::string_view key) {
  if (access_log_response_trailers_) return getHeader(access_log_response_trailers_, key);
  return getHeader(response_trailers_, key);
}

Pairs Context::getResponseTrailerPairs() {
  if (access_log_response_trailers_) return getHeaderPairs(access_log_response_trailers_);
...
uint32_t Context::httpCall(absl::string_view cluster, const Pairs& request_headers,
  ...
  if (cluster_manager->get(cluster_string) == nullptr) return 0;
  ...
  auto http_request = cluster_manager
...
void Context::httpRespond(const Pairs& response_headers, absl::string_view body, const Pairs& response_trailers) {
...
const StreamInfo::StreamInfo &Context::streamInfo() const {
  if (access_log_stream_info_) return *access_log_stream_info_;
...
void Context::setRequestMetadata(absl::string_view key, absl::string_view serialized_proto_struct) {
  if (!decoder_callbacks_) return;
  decoder_callbacks_->streamInfo().setDynamicMetadata(HttpFilters::HttpFilterNames::get().Wasm, MessageUtil::keyValueStruct(std::string(key), std::string(serialized_proto_struct)));
}
...
void Context::setResponseMetadata(absl::string_view key, absl::string_view serialized_proto_struct) {
  if (!encoder_callbacks_) return;
  encoder_callbacks_->streamInfo().setDynamicMetadata(HttpFilters::HttpFilterNames::get().Wasm, MessageUtil::keyValueStruct(std::string(key), std::string(serialized_proto_struct)));
}
...
Http::FilterHeadersStatus Context::onRequestHeaders() { 
  if (!wasm_->onRequestHeaders_) return Http::FilterHeadersStatus::Continue;
...
Http::FilterDataStatus Context::onRequestBody(int body_buffer_length, bool end_of_stream) {
  if (!wasm_->onRequestBody_) return  Http::FilterDataStatus::Continue;
...
Http::FilterTrailersStatus Context::onRequestTrailers() { 
  if (!wasm_->onRequestTrailers_) return Http::FilterTrailersStatus::Continue;
...
Http::FilterMetadataStatus Context::onRequestMetadata() { 
  if (!wasm_->onRequestMetadata_) return Http::FilterMetadataStatus::Continue;
...
Http::FilterHeadersStatus Context::onResponseHeaders() { 
  if (!wasm_->onResponseHeaders_) return Http::FilterHeadersStatus::Continue;
...
Http::FilterDataStatus Context::onResponseBody(int body_buffer_length, bool end_of_stream) {
  if (!wasm_->onResponseBody_) return  Http::FilterDataStatus::Continue;
...
Http::FilterTrailersStatus Context::onResponseTrailers() { 
  if (!wasm_->onResponseTrailers_) return Http::FilterTrailersStatus::Continue;
...
Http::FilterMetadataStatus Context::onResponseMetadata() { 
  if (!wasm_->onResponseMetadata_) return Http::FilterMetadataStatus::Continue;
...
void Context::onHttpCallResponse(uint32_t token, const Pairs& response_headers,
  ...
  if (!wasm_->onHttpCallResponse_) return;
  ...
  wasm_->onHttpCallResponse_(this, id_, token, headers_ptr, headers_size, body_ptr, body_size, trailers_ptr, trailers_size);
}
...
void Context::onDone() {
  if (wasm_->onDone_) wasm_->onDone_(this, id_);
}
